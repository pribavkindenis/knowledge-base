...
## Получение самой левой единицы в двоичном представлении

Принцип тот же, что и при нахождении максимальной степени двойки, меньшей, либо равной заданному числу. Необходимо получить число, двоичное представление которого содержит все единицы справа от самого старшего бита.

После прибавляем к получившемуся числу единицу, получая степень двойки, содержащую в двоичном представлении на единицу большее кол-во значащих битов, чем оригинальное число. 

В заключении выполняем один битовый сдвиг вправо.
...
Ниже представлен алгоритм для 32битного знакового числа:

```python
def get_leftmost_one(n: int) -> int:
    n |= n >> 1
    n |= n >> 2
    n |= n >> 4
    n |= n >> 8
